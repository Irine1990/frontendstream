import React from 'react'
import PlaylistNameCard from '../components/PlaylistNameCard'
import CommentCard from '../components/CommentCard'
import VideoCardHorizontal from '../components/VideoCardHorizontal'
import { useParams } from 'react-router-dom'
import { useState, useEffect } from 'react'
import videoService from '../service/video'
import { useDispatch, useSelector } from 'react-redux'
import commentService from '../service/comment'
import timeAgo from '../utils/timeInterval'
import channelService from '../service/channel'
import subscriptionService from '../service/subscription'
import playlistService from '../service/playlist'
import { Link } from 'react-router-dom'
import { setComments } from '../slices/commentSlice'
import { setVideos } from '../slices/videoSlice'

function VideoDetail() {
    const { videoId } = useParams()
    const dispatch = useDispatch();

    //Current User      
    const user = useSelector(state => state.auth.user)
    console.log("User", user)

    //Playlists
    const [playlistInput, setPlaylistInput] = useState("")
    const [playlists, setPlaylists] = useState(null)

    //Video and Owner
    const [video, setVideo] = useState(null)
    const [videoOwner, setVideoOwner] = useState({})
    const [isSubscribed, setIsSubscribed] = useState(false)

    //Videos
    const videos = useSelector(state => state.video.videos)

    //Comments
    const [commentInput, setCommentInput] = useState("")
    const comments = useSelector(state => state.comment.comments)

    const subscriptionHandler = () => {
        subscriptionService.toggleSubscription(videoOwner._id)
            .then((res) => setIsSubscribed(!isSubscribed))
    }

    const createPlaylistHandler = () => {
        playlistService.createPlaylist(playlistInput)
            .then((res) => {
                console.log("Playlists", res)
                setPlaylists([...playlists, res.data])
                setPlaylistInput("")
            })
    }

    const commentHandler = () => {
        commentService.createComment(videoId, commentInput)
            .then((res) => {
                commentService.getAllComments(videoId)
                    .then((res) => {
                        dispatch(setComments(res.data.comments))
                    })
                setCommentInput("")
            })
    }

    useEffect(() => {
        videoService.getVideo(videoId)
            .then((res) => {
                const video = res.data
                video.timeAgo = timeAgo(video.createdAt)
                console.log("Video", video)
                setVideo(video)
            })
            .catch((err) => console.log("Error while fetching video details", err))

        videoService.getAllVideos()
            .then((res) => {
                console.log("All videos ", res.data.docs)
                dispatch(setVideos(res.data.docs))
            })
    }, [])

    useEffect(() => {
        if (video) {
            channelService.getChannelInfo(video.owner)
                .then((res) => {
                    console.log("Owner", res.data)
                    setVideoOwner(res.data)
                    setIsSubscribed(videoOwner.isSubscribed)
                })
                .catch((err) => console.log("Error while fetching channel details", err))
        }
    }, [video, isSubscribed])

    useEffect(() => {
        commentService.getAllComments(videoId)
            .then((res) => {
                console.log("Comments", res.data.comments)
                dispatch(setComments(res.data.comments))
            })
    }, [video])

    useEffect(() => {
        if (user) {
            playlistService.getUserPlaylists(user?._id)
                .then((res) => {
                    setPlaylists(res.data.docs)
                })
        }
        else { setPlaylists(null) }
    }, [user])

    return (
        <section className="w-full pb-[70px] sm:ml-[70px] sm:pb-0">
            <div className="flex w-full flex-wrap gap-4 p-4 lg:flex-nowrap">
                <div className="col-span-12 w-full">
                    <div className="relative mb-4 w-full pt-[56%]">
                        <div className="absolute inset-0">
                            {
                                video?.videoFile &&
                                <video
                                    className="h-full w-full"
                                    controls
                                    autoPlay
                                    muted>
                                    <source
                                        src={video?.videoFile}
                                        type="video/mp4" />
                                </video>
                            }
                        </div>
                    </div>
                    <div
                        className="group mb-4 w-full rounded-lg border p-4 duration-200 hover:bg-white/5 focus:bg-white/5"
                        role="button"
                        tabIndex="0">
                        <div className="flex flex-wrap gap-y-2">
                            <div className="w-full md:w-1/2 lg:w-full xl:w-1/2">
                                <h1 className="text-lg font-bold">{video?.title}</h1>
                                <p className="flex text-sm text-gray-200">{`132 views | ${video?.timeAgo}`}</p>
                            </div>
                            <div className="w-full md:w-1/2 lg:w-full xl:w-1/2">
                                <div className="flex items-center justify-between gap-x-4 md:justify-end lg:justify-between xl:justify-end">
                                    <div className="flex overflow-hidden rounded-lg border">
                                        <button
                                            className="group/btn flex items-center gap-x-2 border-r border-gray-700 px-4 py-1.5 after:content-[attr(data-like)] hover:bg-white/10 focus:after:content-[attr(data-like-alt)]"
                                            data-like="3050"
                                            data-like-alt="3051">
                                            <span className="inline-block w-5 group-focus/btn:text-[#ae7aff]">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    strokeWidth="1.5"
                                                    stroke="currentColor"
                                                    aria-hidden="true">
                                                    <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                                                        d="M6.633 10.5c.806 0 1.533-.446 2.031-1.08a9.041 9.041 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75A2.25 2.25 0 0116.5 4.5c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H13.48c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H5.904M14.25 9h2.25M5.904 18.75c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368a12 12 0 01-.521-3.507c0-1.553.295-3.036.831-4.398C3.387 10.203 4.167 9.75 5 9.75h1.053c.472 0 .745.556.5.96a8.958 8.958 0 00-1.302 4.665c0 1.194.232 2.333.654 3.375z"></path>
                                                </svg>
                                            </span>
                                        </button>
                                        <button
                                            className="group/btn flex items-center gap-x-2 px-4 py-1.5 after:content-[attr(data-like)] hover:bg-white/10 focus:after:content-[attr(data-like-alt)]"
                                            data-like="20"
                                            data-like-alt="21">
                                            <span className="inline-block w-5 group-focus/btn:text-[#ae7aff]">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    strokeWidth="1.5"
                                                    stroke="currentColor"
                                                    aria-hidden="true">
                                                    <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                                                        d="M7.5 15h2.25m8.024-9.75c.011.05.028.1.052.148.591 1.2.924 2.55.924 3.977a8.96 8.96 0 01-.999 4.125m.023-8.25c-.076-.365.183-.75.575-.75h.908c.889 0 1.713.518 1.972 1.368.339 1.11.521 2.287.521 3.507 0 1.553-.295 3.036-.831 4.398C20.613 14.547 19.833 15 19 15h-1.053c-.472 0-.745-.556-.5-.96a8.95 8.95 0 00.303-.54m.023-8.25H16.48a4.5 4.5 0 01-1.423-.23l-3.114-1.04a4.5 4.5 0 00-1.423-.23H6.504c-.618 0-1.217.247-1.605.729A11.95 11.95 0 002.25 12c0 .434.023.863.068 1.285C2.427 14.306 3.346 15 4.372 15h3.126c.618 0 .991.724.725 1.282A7.471 7.471 0 007.5 19.5a2.25 2.25 0 002.25 2.25.75.75 0 00.75-.75v-.633c0-.573.11-1.14.322-1.672.304-.76.93-1.33 1.653-1.715a9.04 9.04 0 002.86-2.4c.498-.634 1.226-1.08 2.032-1.08h.384"></path>
                                                </svg>
                                            </span>
                                        </button>
                                    </div>
                                    <div className="relative block">
                                        <button className="peer flex items-center gap-x-2 rounded-lg bg-white px-4 py-1.5 text-black">
                                            <span className="inline-block w-5">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    strokeWidth="1.5"
                                                    stroke="currentColor"
                                                    aria-hidden="true">
                                                    <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                                                        d="M12 10.5v6m3-3H9m4.06-7.19l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z"></path>
                                                </svg>
                                            </span>
                                            Save
                                        </button>
                                        <div className="absolute right-0 top-full z-10 hidden w-64 overflow-hidden rounded-lg bg-[#121212] p-4 shadow shadow-slate-50/30 hover:block peer-focus:block">
                                            <h3 className="mb-4 text-center text-lg font-semibold">Save to playlist</h3>
                                            <ul className="mb-4">
                                                {user ?
                                                    (
                                                        playlists ?
                                                            playlists?.map((playlist) => (
                                                                <li key={playlist._id} className="mb-2 last:mb-0">
                                                                    <PlaylistNameCard playlist={playlist} />
                                                                </li>
                                                            )) : ""
                                                    ) :
                                                    <p className='text-red-500'> Login First </p>
                                                }
                                            </ul>
                                            <div className="flex flex-col">
                                                <label
                                                    htmlFor="playlist-name"
                                                    className="mb-1 inline-block cursor-pointer">
                                                    Name
                                                </label>
                                                <input
                                                    className="w-full rounded-lg border border-transparent bg-white px-3 py-2 text-black outline-none focus:border-[#ae7aff]"
                                                    id="playlist-name"
                                                    placeholder="Enter playlist name"
                                                    value={playlistInput}
                                                    onChange={(e) => setPlaylistInput(e.target.value)}
                                                />
                                                <button onClick={createPlaylistHandler} className="mx-auto mt-4 rounded-lg bg-[#ae7aff] px-4 py-2 text-black">Create new playlist</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="mt-4 flex items-center justify-between">
                            <div className="flex items-center gap-x-4">
                                <div className="mt-2 h-12 w-12 shrink-0">
                                    <img
                                        src={videoOwner?.avatar}
                                        alt="reactpatterns"
                                        className="h-full w-full rounded-full" />
                                </div>
                                <div className="block">
                                    <p className="text-gray-200">{videoOwner?.fullname}</p>
                                    <p className="text-sm text-gray-400">{`${videoOwner?.subscriberCount} subscribers`}</p>
                                </div>
                            </div>
                            <div className="block">
                                {user ?
                                    <button
                                        onClick={subscriptionHandler}
                                        className="group/btn mr-1 flex w-full items-center gap-x-2 bg-[#ae7aff] px-3 py-2 text-center font-bold text-black shadow-[5px_5px_0px_0px_#4f4e4e] transition-all duration-150 ease-in-out active:translate-x-[5px] active:translate-y-[5px] active:shadow-[0px_0px_0px_0px_#4f4e4e] sm:w-auto">
                                        {!isSubscribed ?
                                            <span className="group-focus/btn:block">Subscribe</span> :
                                            <span className="group-focus/btn:block">Subscribed</span>
                                        }
                                    </button> :
                                    <Link to={"/login"} className='text-red-500'>Login to Subscribe</Link>
                                }
                            </div>
                        </div>
                        <hr className="my-4 border-white" />
                        <div className="h-5 overflow-hidden group-focus:h-auto">
                            <p className="text-sm">
                                {video?.description}
                            </p>
                        </div>
                    </div>
                    <button className="peer w-full rounded-lg border p-4 text-left duration-200 hover:bg-white/5 focus:bg-white/5 sm:hidden"><h6 className="font-semibold">573 Comments...</h6></button>
                    <div
                        className="fixed inset-x-0 top-full z-[60] h-[calc(100%-69px)] overflow-auto rounded-lg border bg-[#121212] p-4 duration-200 hover:top-[67px] peer-focus:top-[67px] sm:static sm:h-auto sm:max-h-[500px] lg:max-h-none">
                        <div className="block">
                            <h6 className="mb-4 font-semibold">{`${comments?.length} comments`}</h6>
                            <input
                                type="text"
                                disabled={!user}
                                value={commentInput}
                                onChange={(e) => setCommentInput(e.target.value)}
                                className={`w-full rounded-lg border bg-transparent px-2 py-1 placeholder-white ${user ? "" : "bg-gray-700 opacity-50"}`}
                                placeholder={user ? "Add a comment" : "Login to add a comment"} />
                            <button onClick={commentHandler} className="mt-2 rounded-lg bg-[#ae7aff] px-2 py-1 text-black">Comment</button>
                        </div>
                        <hr className="my-4 border-white" />
                        {
                            comments.map((comment) => (
                                <CommentCard key={comment._id} comment={comment} />
                            ))
                        }
                    </div>
                </div>
                <div className="col-span-12 flex w-full shrink-0 flex-col gap-3 lg:w-[350px] xl:w-[400px]">
                    {
                        videos
                            .filter(video => video.id !== videoId)
                            .map(video => (
                                <VideoCardHorizontal key={video._id} video={video} />
                            ))
                    }
                </div>
            </div>
        </section>)
}

export default VideoDetail